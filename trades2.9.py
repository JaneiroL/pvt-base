...
# RR Gate (nur planned_rr)  ✅ Early Gate, aber identisch zum alten Post-Filter via sim!=None consumption
RR_FILTER_MIN = 0.90
RR_FILTER_MAX = 1.30
...
# PATCHED: Sortierung nach entry_time_candidate statt wd_first_time
# -----------------------------------
def run_trades_for_wd_dfs(
    ...
    # Initial sort - this is just for grouping consistency, not for processing order
    wd = wd.sort_values(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        # ========================================
        # PATCH START: Entry-Time-Based Sorting
        # ========================================
        
        # Step 1: Build candidate list with entry_time_candidate for each WD
        wd_candidates = []
        for idx, row in g.iterrows():
            ...
            if entry_idx is not None:
                entry_time_candidate = ltf_df.loc[entry_idx, "time"]
                wd_candidates.append({
                    "entry_time_candidate": entry_time_candidate,
                    "wd_source_ltf": str(row.get("wd_source_ltf", "")).upper(),
                    "wd_first_time": row["wd_first_time"],
                    "wd_low": float(row["wd_low"]),
                    "row_index": idx,
                    "row": row,
                    "entry_idx": entry_idx,
                })
        ...
        # Step 2: Sort by entry_time_candidate with tie-breaks
        wd_candidates_sorted = sorted(
            wd_candidates,
            key=lambda x: (
                x["entry_time_candidate"],      # Primary: earliest entry time
                x["wd_source_ltf"],              # Tie-break 1: alphabetical
                x["wd_first_time"],              # Tie-break 2: earlier wd_first_time
                x["wd_low"],                     # Tie-break 3: lower wd_low
            )
        )
        ...
        # Step 3: Process WDs in the new sorted order
        for cand in wd_candidates_sorted:
            if pivot_consumed:
                break
            ...
            # ✅ IDENTISCH zum alten Post-Filter: Pivot wird erst "verbraucht", wenn ein Trade wirklich existiert (sim != None).
            sim = simulate_trade(
                ltf_df, entry_idx, direction, entry_price, tp_price, sl_price, planned_rr_pos
            )
            if sim is None:
                continue  # wichtig: Pivot nicht konsumieren, nächste Zone darf versuchen

            # ab hier: Trade existiert -> Pivot wäre im alten System geloggt worden (und ggf. später rausgefiltert)
            pivot_consumed = True

            # ✅ EARLY Gate (identisches Verhalten zum alten Post-Filter):
            # Trade existiert, aber außerhalb [0.90..1.30] => nicht loggen, Pivot dennoch verbraucht.
            if planned_rr_pos < RR_FILTER_MIN or planned_rr_pos > RR_FILTER_MAX:
                break
            ...
            break  # Pivot verbraucht (Trade existiert & wurde geloggt)
    ...
    # ❌ Post-Filter entfernt: Gate passiert jetzt "early", aber identisch zum alten Ergebnis durch sim!=None consumption.
    return df_out
...
def summarize_trades(df_trades: pd.DataFrame, out_path: Path) -> None:
    if df_trades.empty:
        print("⚠️ Keine Trades vorhanden für Summary.")
        return
    ...
    print(f"Ø planned_rr (positiv): {avg_planned_rr_tot:.3f} | Gate: [{RR_FILTER_MIN:.2f} .. {RR_FILTER_MAX:.2f}]")
