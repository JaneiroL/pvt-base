...
def load_pivots(path: Path, tf_tags: list[str]) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "first_touch_time": (to_naive_datetime(df[first_touch_col]) if first_touch_col is not None else pd.NaT),
    })
    ...
    out = out[mask].dropna(subset=["first_candle_time","second_candle_time","gap_low","gap_high","wick_diff_low","wick_diff_high"]).reset_index(drop=True)
...
def scan_pair_for_pivot_outside(
    ...
    for i in range(len(dfw) - 1):
        ...
        # 1) Zone muss komplett im großen Pivot liegen
...
def run_mode(
    ...
    if not all_rows:
        print(f"⚠️ Keine gültigen OUTSIDE Wick-Differences gefunden für {mode_name}.")
        if skipped:
...
def main() -> None:
    ...
    # OUTSIDE Outputs in eigene Unterordner (damit nix überschrieben wird)
    run_mode(
        base=base,
        mode_name="W",
