...
NEU (auf deinen Wunsch):
- LTF (WickDiff-Timeframe) ist EINMAL auswählbar und gilt dann für ALLE HTF-Modi:
  1h / 4h / 12h / daily
"""
...
# LTF Auswahl (einmalig, gilt für alle HTF)
# key -> (relative folder, label)
LTF_SPECS = {
    "H1":  (Path("time frame data") / "1h data",    "H1"),
    "H4":  (Path("time frame data") / "4h data",    "H4"),
    "H12": (Path("time frame data") / "12h",        "H12"),
    "D1":  (Path("time frame data") / "daily data", "D1"),
}
...
def latest_pivots_csv(pivots_dir: Path) -> Optional[Path]:
    ...
    # Primär: klassisches pivots_gap_* Muster
    cands = [
        p for p in pivots_dir.glob("*.csv")
        if ("pivot" in p.name.lower() and "gap" in p.name.lower())
    ]

    # Fallback: irgendeine CSV im Ordner (wichtig, falls Naming abweicht)
    if not cands:
...
# -----------------------------
def run_mode_variant(base: Path, mode: str, variant: str, ltf_rel: Path, ltf_label: str) -> pd.DataFrame:
...
def parse_ltf_input(s: str) -> str:
    if s is None:
        raise ValueError("Ungültige LTF-Eingabe. Erlaubt: 1h, 4h, 12h, daily")
    raw = s.strip().lower()
    if not raw:
        raise ValueError("Ungültige LTF-Eingabe. Erlaubt: 1h, 4h, 12h, daily")

    alias = {
        "1h": "H1", "h1": "H1", "1": "H1", "hour": "H1", "1hour": "H1",
        "4h": "H4", "h4": "H4", "4": "H4", "4hour": "H4",
        "12h": "H12", "h12": "H12", "12": "H12", "12hour": "H12",
        "d": "D1", "d1": "D1", "day": "D1", "daily": "D1", "1d": "D1",
    }
    if raw not in alias:
        raise ValueError("Ungültige LTF-Eingabe. Erlaubt: 1h, 4h, 12h, daily")
    return alias[raw]
...
def prompt_ltf() -> str:
    print("\nWelchen LTF (WickDiff-Timeframe) willst du für ALLE HTF-Modi verwenden?")
    print("  1 = 1h")
    print("  2 = 4h")
    print("  3 = 12h")
    print("  4 = daily")
    v = input("Eingabe (1/2/3/4): ").strip()
    m = {"1": "H1", "2": "H4", "3": "H12", "4": "D1"}
    if v not in m:
        raise ValueError("Ungültige Eingabe. Erlaubt: 1,2,3,4")
    return m[v]
...
def main():
    ...
    ap.add_argument("--ltf", type=str, default="", help="LTF für WickDiffs: 1h | 4h | 12h | daily (leer => Prompt).")
    args = ap.parse_args()
    ...
    # LTF Auswahl (einmalig, gilt dann für alle HTF)
    ltf_in = args.ltf.strip()
    if not ltf_in:
        try:
            ltf_key = prompt_ltf()
        except Exception as e:
        ...
        try:
            ltf_key = parse_ltf_input(ltf_in)
        except Exception as e:
            print(f"❌ {e}")
            sys.exit(1)

    ltf_rel, ltf_label = LTF_SPECS[ltf_key]
    ...
    for mode in modes:
        # OutputPrefix bleibt im selben Stil, aber LTF ist jetzt dynamisch
        out_prefix = f"{mode}_{ltf_label}"

        for v in variants_to_run:
            vname = VARIANT_NAMES[v]
            df = run_mode_variant(base, mode, vname, ltf_rel=ltf_rel, ltf_label=ltf_label)
            if df.empty:
                print(f"⚠️ Keine Wickdiffs gefunden für {mode} | {vname} | LTF={ltf_label}")
                continue
            ...
            # Output-Ordner eindeutig pro Variante (Style bleibt identisch)
            ...
            print(f"\n✅ Wickdiffs gespeichert: TF={mode} | Variant={vname} | LTF={ltf_label} | n={len(df)}")
