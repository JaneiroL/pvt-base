...
def _parse_wd_meta_from_dirname(dirname: str) -> Tuple[Optional[str], str, Optional[str]]:
    """
    Versucht aus einem Wickdiff-Unterordner-Namen Modus, Variante und LTF herauszulesen.
    Rückgabe: (mode, variant, wd_ltf)
    ...
    # Mode (Reihenfolge wichtig, damit 2W nicht als W erkannt wird)
    mode = None
    if "2W" in tokens:
        mode = "2W"
    elif "3D" in tokens:
...
def load_wickdiff_csv(path: Path, mode: str, variant: str, wd_ltf: str) -> pd.DataFrame:
    """
    Lädt eine einzelne Wickdiff-CSV – robust gegen verschiedene Spaltennamen
    und vereinheitlicht die wichtigsten Felder.
    """
    ...
    first_candle_col  = _pick_any(df_raw, ["pivot_first_time","first_candle_time","htf_first_candle_time",
                                           "weekly_first_candle_time","3day_first_candle_time","2w_first_candle_time","m_first_candle_time"])
    second_candle_col = _pick_any(df_raw, ["pivot_second_time","second_candle_time","htf_second_candle_time",
    ...
    touch_col = _pick_any(df_raw, ["pivot_touch_time","first_touch_time","htf_first_touch_time",
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # OUTSIDE-Check nur für OUTSIDE-Varianten (fail-soft)
...
def summarize_trades(df_trades: pd.DataFrame, out_path: Path) -> None:
    ...
    rows = []
    for (pair, mode, variant, wd_ltf, entry_ltf), g in df.groupby(["pair","mode","wd_variant","wd_source_ltf","entry_ltf"]):
        n = len(g)
    ...
    summary = pd.DataFrame(rows).sort_values(["pair","mode","wd_variant","wd_source_ltf","entry_ltf"]).reset_index(drop=True)
...
def main() -> None:
    ...
    user_ltf = input("LTF-Eingabe: ").strip().upper()
    ...
    if user_ltf in {"", "AUTO"}:
        ...
        if user_ltf not in tf_map:
            raise ValueError(f"Unbekanntes LTF '{user_ltf}'. Erlaubt: H1, H4, H12, D1, 3D, AUTO.")
        # Gleiches LTF für alle HTFs
        entry_specs = {mode: tf_map[user_ltf] for mode in ["3D","W","2W","M"]}
        print(f"➡️ Fixes Entry-LTF für alle HTFs: {user_ltf}")
