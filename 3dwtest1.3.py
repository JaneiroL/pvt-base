...
def detect_pivots_for_pair(
    ...
    for i in range(len(df) - 1):
        ...
        # --- Pivot-Zone + Wick-Diff sauber nach deiner Logik ---

        if ptype == "long":
            # Große Pivot-Zone (unten -> oben):
            gap_low = float(min(l1, l2))          # tiefster Wick
            gap_high = chosen_price               # Pivot-Linie (Open der Bull-Kerze)

            # Wick-Diff-Zone: unterer Teil innerhalb der großen Zone
            wick_diff_low = gap_low               # untere Kante = Pivot-Low
            wick_diff_high = float(max(l1, l2))   # anderer Wick nach oben

        else:  # ptype == "short"
            # Große Pivot-Zone (unten -> oben):
            gap_low = chosen_price                # Pivot-Linie (Open der Bear-Kerze)
            gap_high = float(max(h1, h2))         # höchster Wick

            # Wick-Diff-Zone: oberer Teil innerhalb der großen Zone
            wick_diff_low = float(min(h1, h2))    # anderer Wick nach unten
            wick_diff_high = gap_high             # obere Kante = Pivot-High
