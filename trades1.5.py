...
RR_MIN = 0.95
RR_MAX = 1.49
MIN_SL_PIPS = 50  # generischer Fallback (falls alles fehlt)
...
# -----------------------------------
# Vola-Index laden (für Nicht-Weekly)
# -----------------------------------
def load_vola_index(base: Path) -> Dict[str, pd.DataFrame]:
    ...
    if not vola_map:
        print(f"⚠️ Konnte keine gültigen Vola-Sheets finden – benutze statische {MIN_SL_PIPS} Pips.")
    return vola_map

def get_min_sl_pips_from_vola(vola_map: Dict[str, pd.DataFrame], pair6: str, entry_time: pd.Timestamp) -> float:
...
# -----------------------------------
# Weekly: Volatility-Factor-Pips (50 * factor)
# -----------------------------------
def _resolve_weekly_factor_xlsx(base: Path) -> Optional[Path]:
    """
    Sucht die Volatility_Faktor_Pips.xlsx robust in typischen Repo-Pfaden.
    (Fix für dein aktuelles Pathing-Problem)
    """
    candidates = [
        base / "time frame data" / "Volatility Faktor Pips" / "Volatility_Faktor_Pips.xlsx",
        base / "time frame data" / "Volatility Faktor Pips" / "Volatility Faktor Pips.xlsx",
        base / "time frame data" / "Volatility_Faktor_Pips.xlsx",
        base / "time frame data" / "Volatility Faktor Pips.xlsx",
        base / "Volatility_Faktor_Pips.xlsx",
        base / "Volatility Faktor Pips.xlsx",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def load_weekly_volatility_factors(base: Path) -> Dict[str, float]:
    """
    Liest Volatility_Faktor_Pips.xlsx:
      - Spalte: Currency Pair
      - Spalte: Volatility Factor
    Rückgabe: dict[pair6] = factor (float)
    """
    xlsx_path = _resolve_weekly_factor_xlsx(base)
    if xlsx_path is None:
        print("⚠️ Volatility_Faktor_Pips.xlsx nicht gefunden – Weekly nutzt Faktor=1.0 (=> 50 Pips).")
        return {}

    try:
        df = pd.read_excel(xlsx_path, sheet_name=0, engine="openpyxl")
    except Exception as e:
        print(f"⚠️ Fehler beim Laden {xlsx_path}: {e} – Weekly nutzt Faktor=1.0 (=> 50 Pips).")
        return {}

    pair_col = _pick_any(df, ["currency pair", "pair", "symbol", "instrument"])
    factor_col = _pick_any(df, ["volatility factor", "vola factor", "faktor", "factor"])
    if pair_col is None or factor_col is None:
        print(f"⚠️ {xlsx_path.name} hat nicht die erwarteten Spalten – Weekly nutzt Faktor=1.0 (=> 50 Pips).")
        return {}

    out: Dict[str, float] = {}
    for _, row in df.iterrows():
        p = pair_code_from_str(row[pair_col])
        if p not in PAIRS_28:
            continue
        f = pd.to_numeric(row[factor_col], errors="coerce")
        if not np.isfinite(f) or f <= 0:
            continue
        out[p] = float(f)

    if not out:
        print(f"⚠️ Keine gültigen Faktoren in {xlsx_path.name} gefunden – Weekly nutzt Faktor=1.0 (=> 50 Pips).")
    else:
        print(f"✅ Weekly-Faktoren geladen: {len(out)} Pairs aus {xlsx_path}")

    return out

def get_weekly_min_sl_pips(pair6: str, weekly_factors: Dict[str, float]) -> float:
    """
    Weekly-Min-SL in Pips: 50 * factor. Falls factor fehlt: 50.
    """
    f = weekly_factors.get(pair6.upper(), 1.0)
    return float(50.0 * f)
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # ✅ DEINE REGEL:
            # Weekly: 50 * VolatilityFactor (Excel)
            # Andere Modes: alte Vola-Index Logik
            if mode.upper() == "W":
                dyn_min_sl_pips = get_weekly_min_sl_pips(pair6, weekly_factors)
            else:
...
def main() -> None:
    ...
    # Vola Index (für Nicht-Weekly)
    vola_map = load_vola_index(base)

    # ✅ Weekly Factor Excel (für Weekly)
    ...
    if user_ltf_norm == "AUTO":
        ...
        if user_ltf_norm not in tf_map:
            raise ValueError("Unbekanntes LTF. Erlaubt: H1, H4, H12, D1, 3D, AUTO.")
