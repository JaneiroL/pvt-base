...
# NUR die eine, die du wirklich schon im Block-2 Code hattest:
SPECIAL_PAIR_FIX = {
    "OANDAG": "GBPNZD",
}
...
def infer_pair_from_text(txt: str):
    """
    1) SPECIAL_PAIR_FIX (nur OANDAG->GBPNZD)
    2) eines der 28 Paare
    3) generischer 6-Buchstaben-Block (Fallback)
    """
    up = re.sub(r"[^A-Z]", "", str(txt).upper())

    # exakt wie im anderen Code: Sonderfix zuerst
...
def detect_pivots_for_pair(pair: str, timeframe: str, df: pd.DataFrame, start_dt, end_dt) -> pd.DataFrame:
    ...
    for i in range(len(df) - 1):
        ...
        if ptype == "long":
            # WD unten im Gap: untere Kante = Gap-Low, obere Kante = höherer unterer Wick
            wick_diff_low = gap_low_n
            wick_diff_high = float(max(l1, l2))
        else:
            # WD oben im Gap: obere Kante = Gap-High, untere Kante = tieferer oberer Wick
...
def parse_tf_input(s: str) -> set[str]:
    """
    Erlaubt: W, 3D, 2W, M, All/Both, Kombis mit Leerzeichen/Komma/+.
    """
    raw = s.strip()
    if not raw:
...
def tf_to_outsub(tf: str) -> str:
    # exakt deine Output-Ordnernamen
