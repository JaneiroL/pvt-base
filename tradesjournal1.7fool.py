...
# -----------------------------------
# OHLC-Reader (wie in wick_diffs_run)
# -----------------------------------
...
# -----------------------------------
# NEUE REGEL: Pivot-TP-Invalidation
# -----------------------------------
def compute_pivot_tp_level(
    ...
    """
    Berechnet das "theoretische" TP-Level des großen Pivots
    (Pivot-Breite 1:1 projiziert):
...
def pivot_invalidated_by_tp_before_wd(
    ...
    """
    Neue Invalidation-Regel:

    Ab pivot_touch_time bis end_time (6 Tage 3D / 14 Tage W):

    - Wenn der Markt ZUERST das theoretische Pivot-TP-Level erreicht,
      OHNE dass davor die WD-Zone berührt wurde → der gesamte Pivot ist invalid.
    - Wenn die WD-Zone zuerst berührt wird → dieser Fall ist erledigt,
      Pivot bleibt bzgl. dieser Regel gültig.

    Das verhindert:
      großer Pivot → erster Touch → direkter Bounce zum TP-Level
      → späterer WD-Touch würde sonst fälschlich einen Entry erlauben.
    """
    ...
    for _, row in df.iterrows():
        ...
        # Sobald WD zuerst kommt → kein "Bounce-only-zum-TP"-Fall
        if touched_wd:
            return False

        # TP erreicht, ohne dass WD vorher berührt wurde → Pivot invalid
        if hit_tp:
            return True
...
def find_entry_candle(
    ...
    """
    Sucht ab start_time (erstem Pivot-Touch) bis end_time (6/14 Tage)
    die erste gültige Entry-Kerze in der WD-Zone.

    Regeln:
    - Kill-Kerze: Candle-Körper komplett durch die Zone → Pivot für Entries tot.
    - Gültiger Entry:
      Long:  Candle berührt Zone, schließt über Zone, Körper komplett über High.
      Short: Candle berührt Zone, schließt unter Zone, Körper komplett unter Low.
    """
...
def compute_tp_sl(
    ...
    """
    SL/TP-Regeln:
    - TP = 1:1-Extension der Pivot-Breite (gleiches Level wie Pivot-TP),
    - SL mindestens:
        * Pivot-1.1-Baseline
        * 50 Pips
        * so groß, dass RR_MAX (1.49) nicht überschritten wird
    - Resultierendes RR muss zwischen RR_MIN (0.95) und RR_MAX (1.49) liegen.
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
...
def simulate_trade(
    ...
    """
    Simuliert Trade im LTF:
    - Prüft Kerzen nach Entry
    - Wenn in einer Kerze TP und SL getroffen → konservativ SL zuerst
    - Sonst erstes Level (TP/SL) entscheidet.
    """
    n = len(ltf)
...
def run_mode(
    ...
    # Optional: nur bestimmte Paare
    if pair_filter is not None:
        wd = wd[wd["pair6"].isin(pair_filter)].copy()

    # Optional: nur Pivots in einem Zeitfenster (über pivot_first_time)
    ...
    # Zeitliche Gültigkeit: 6 Tage (3D) bzw. 14 Tage (W)
    ...
    for _, row in wd.iterrows():
        ...
        if pd.isna(pivot_touch_time):
            # Ohne definierten Touch macht der Rest keinen Sinn
            continue

        end_time = pivot_touch_time + pd.Timedelta(days=max_days)

        # 1) Neue Invalidation-Regel: Bounce Pivot → TP-Level, bevor WD berührt wird
        ...
        # 2) Entry-Suche in der WD-Zone innerhalb des gültigen Zeitfensters
