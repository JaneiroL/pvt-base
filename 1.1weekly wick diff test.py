...
def scan_pair_for_pivot(
    ...
    for i in range(len(dfw) - 1):
        ...
        # -----------------------------------
        # UNBERÜHRT-REGEL mit 24h-Vorlauf
        # -----------------------------------
        if pd.notna(w_touch):
            w_touch_ts = pd.Timestamp(w_touch)
            buffer_end = w_touch_ts - pd.Timedelta(days=1)
            # Nur prüfen, wenn der gepufferte Endzeitpunkt nach unserer zweiten Kerze liegt
            if buffer_end > t_b:
                end_check = buffer_end
                if any_touch_between(ltf, z_lo, z_hi, t_b, end_check):
                    continue
            # else: kein Unberührt-Check mehr nötig (Zone entsteht innerhalb der 24h vor dem offiziellen First Touch)
        else:
            # Kein First-Touch bekannt -> wie bisher bis Datenende unberührt
            end_check = data_end
            if any_touch_between(ltf, z_lo, z_hi, t_b, end_check):
                continue
...
def main() -> None:
    ...
    if not all_rows:
        print("⚠️ Keine gültigen Wick-Differences gefunden "
              "(Richtung, komplett im Gap, ≤19%, unberührt bis (FirstTouch-24h)/Datenende).")
        if skipped:
