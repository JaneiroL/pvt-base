...
SPECIAL_PAIR_FIX = {
    # Beispiel: falls irgendwo wirklich mal exakt "OANDAG" als 6-Letter-Code auftaucht:
    "OANDAG": "GBPNZD",
}
...
def extract_pair6_from_any_text(txt: str) -> str | None:
    ...
    if m:
        ...
        return code if code in PAIRS_28 else code

    # 2) direkter Treffer PAIRS_28 (im A-Z-only Stream)
...
def find_ltf_files_map(ltf_dir: Path) -> dict[str, Path]:
    """
    Mappt 6-Letter-Paircode -> Pfad (erste gefundene Datei gewinnt).
    """
    mp: dict[str, Path] = {}
    ...
    for p in ltf_dir.rglob("*"):
        if p.is_file() and p.suffix.lower() in {".csv", ".xlsx"}:
            ...
            if code and len(code) == 6 and code.upper() in PAIRS_28 and code not in mp:
                mp[code.upper()] = p
    return mp


# -----------------------------------
# Pivot-CSV laden (W / 3D / 2W / M)
# -----------------------------------
...
def load_pivots(path: Path, tf_tags: list[str]) -> pd.DataFrame:
    ...
    # akzeptiere wick_diff_* oder wd_* (falls du später umbenennst)
    wick_low = None
    wick_high = None
    ...
    # Filter auf gewünschtes TF (mehrere Tags erlaubt)
    tf_upper = out["timeframe"].str.upper()
    mask = False
    for tag in tf_tags:
...
def scan_pair_for_pivot(
    ...
    # Toleranz für "exakt matcht" – ABSOLUT in Preis
    # 1e-6 ist ok für FX (5dp) und JPY (3dp) auch; wenn du willst später dynamisch pip_size rein.
    TOL = 1e-6
    ...
    for i in range(len(dfw) - 1):
        ...
        #    - Grenzfall: eine Grenze matcht relevante HTF-Kante
        inside_wd = (z_lo >= wd_lo and z_hi <= wd_hi)

        if ptype == "long":
            # long: relevante HTF-Kante ist wd_hi (näher am Pivot-Preis)
            match_edge = (abs(z_lo - wd_hi) <= TOL) or (abs(z_hi - wd_hi) <= TOL)
        else:
            # short: relevante HTF-Kante ist wd_lo
...
# -----------------------------------
# Modus ausführen (W / 3D / 2W / M)
# -----------------------------------
def run_mode(
    base: Path,
    mode_name: str,           # z.B. "W", "3D", "2W", "M"
    piv_dir: Path,
    piv_pattern: str,
    tf_tags: list[str],       # filter tags in pivot CSV timeframe column
    ltf_dir: Path,
    ltf_label: str,
    days_win: int,
    out_sub: str,             # folder name under outputs/wickdiffs
    buffer_hours_before_touch: int,
) -> None:
    ...
    if not all_rows:
        print(f"⚠️ Keine gültigen Wick-Differences gefunden für {mode_name} "
              f"(Richtung, HTF-Gap, HTF-WickDiff-Alignment, ≤{int(MAX_REL_WIDTH*100)}%, Unberührtheit).")
        if skipped:
...
# -----------------------------------
# Main – läuft automatisch alle 4 Ketten
# -----------------------------------
def main() -> None:
    base = Path(__file__).resolve().parent

    # Hinweis: Pivot-Ordner-Struktur nach deinem Screenshot:
    # outputs/pivots/{W,3D,2Weekly,Monthly}
    # time frame data/{4h data,1h data,daily data,3D}
    #
    # Output-Struktur nach deinem Screenshot:
    # outputs/wickdiffs/{W→H4,3D→H1,2W→1D,M→3D}
    ...
    # 3) 2W → 1D (neu)
    # First-touch-Regel "Weekly verdoppelt": Puffer 48h.
    # days_win proportional zu Weekly ~ 27 Tage (2*13 + 1).
    run_mode(
        base=base,
        mode_name="2W",
    ...
    # 4) M → 3D (neu)
    # Monthly: grob 4x Weekly; Puffer 96h als Startwert.
    # days_win grob ~ 55 Tage (4*13 + 3).
    run_mode(
        base=base,
        mode_name="M",
