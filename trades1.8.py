...
# -----------------------------------
# Dynamische MIN_SL Pips je HTF-Mode (v1.2: User konfigurierbar in 0.1-Schritten)
# -----------------------------------
...
def _sum_last_days_fractional(hist: pd.DataFrame, days: float) -> Optional[float]:
    """
    Summiert die letzten 'days' Daily-Vola-Tage.
    Unterstützt 0.1-Schritte:
      3.5 -> last1 + last2 + last3 + 0.5*last4
      0.1 -> 0.1*last1
    """
    if days is None:
        return None
    ...
    if frac > 1e-12:
        # letztes Element der weights ist der älteste in tail_newest_first
        # Beispiel 3.5 Tage => need=4 => weights [1,1,1,0.5]
        weights[-1] = frac
...
def get_min_sl_pips_from_vola_by_mode(
    ...
    apply_weekly_factor: bool = True,
) -> float:
    """
    MIN-SL in Pips aus Daily-Vola – komplett adaptiv über vola_days_by_mode.

    - Für jeden Mode wird eine Dezimalzahl 'VOL-Tage' erwartet (0.1 Steps).
    - 0.0 oder 'non' => deaktiviert -> Fallback MIN_SL_PIPS
    - Weekly kann optional zusätzlich mit Weekly-Faktor multipliziert werden.
    """
    ...
    days = None
    if vola_days_by_mode:
        days = vola_days_by_mode.get(mode)

    # deaktiviert oder nicht gesetzt => Fallback
    ...
    if mode == "W" and apply_weekly_factor:
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # ✅ v1.2: MIN-SL in Pips abhängig vom HTF-Mode (User-Input in 0.1 Schritten)
            dyn_min_sl_pips = get_min_sl_pips_from_vola_by_mode(
                ...
                apply_weekly_factor=apply_weekly_factor,
            )
            ...
            # ✅ gewünschte VOL-Spalte: die verwendete Kommazahl (je Mode)
...
def main() -> None:
    ...
    # ✅ NEU: User fragt die VOL-Konfiguration pro Mode ab (0.1 Steps)
    ...
    # Weekly-Faktor toggle (optional, Default: an)
    raw_wf = input("Weekly-Faktor auf W anwenden? (Enter=ja, 'n'=nein): ").strip().lower()
    apply_weekly_factor = not (raw_wf in {"n", "no", "nein", "0", "off"})

    print("\n✅ VOL-Settings aktiv:")
    for m in ["3D", "W", "2W", "M"]:
        print(f"  {m}: {vola_days_by_mode[m]:.1f} Tage" + (" (× Weekly-Faktor)" if (m == "W" and apply_weekly_factor) else ""))
