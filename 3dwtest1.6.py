...
def infer_pair_from_text(txt: str):
    ...
    PRIORITÄT:
    1) Exakter OANDA_{PAIR} Treffer (weil der 1:1 ist)
    2) Suche nach 28er-Paaren als Substring (nur wenn #1 nicht greift)
    3) Fallback: 6-Letter-Blöcke; erst dann SPECIAL_PAIR_FIX anwenden
       (so triggert OANDAG NICHT mehr auf OANDA_GBPUSD, weil #1 vorher gewinnt)
    """
    s = str(txt)

    # 1) OANDA_{PAIR} sauber extrahieren (underscore/space/minus tolerant)
    m = re.search(r"OANDA[\s_\-]*([A-Z]{6})", s.upper())
    if m:
        code = m.group(1)
        if code in PAIRS_28:
            return code
        # falls aus irgendeinem Grund kaputt: mapping versuchen
        code2 = SPECIAL_PAIR_FIX.get(code, code)
        if code2 in PAIRS_28:
            return code2

    # 2) Normalisierte Buchstabenfolge für generische Suche
    up = re.sub(r"[^A-Z]", "", s.upper())

    # 2a) 28er Set (aber ohne SPECIAL-Fix vorher!)
    # Wichtig: sortiert nach Länge/lex, damit wir nicht von Set-Order abhängen.
    for p in sorted(PAIRS_28):
        if p in up:
            return p

    # 3) 6-Letter-Fallback: erst hier Spezialfix
...
def detect_pivots_for_pair(pair: str, timeframe: str, df: pd.DataFrame, start_dt, end_dt) -> pd.DataFrame:
    ...
    for i in range(len(df) - 1):
        ...
        # Große Wick-Diff (deine korrigierte Definition)
        if ptype == "long":
