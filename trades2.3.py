# ============================================================
# TRADES BUILDER – FULL SCRIPT (1/2)
# Änderungen:
# - RR Band: 0.90 .. 1.30
# - Time-Stop: MAX_HOLD_DAYS = 30 -> Trade wird nach 30 Tagen auf Candle-Open geschlossen
# - Concurrency: Spalte active_trades_at_entry + Peak-Concurrency in Summary
# ============================================================
...
MAX_HOLD_DAYS = 30  # Time-Stop (Kalendertage) -> Exit auf Candle-Open sobald Deadline erreicht
...
def _parse_float_01(raw: str, default: float) -> float:
    """
    Erwartet eine Zahl in 0.1-Schritten (z.B. 3,4 oder 3.4).
    Leere Eingabe -> default.
    'non/none/0' -> 0.0 (deaktiviert -> Fallback MIN_SL_PIPS).
    """
    if raw is None:
...
def _parse_wd_meta_from_dirname(dirname: str) -> Tuple[Optional[str], str, Optional[str]]:
    ...
    # WD_LTF korrekt lesen (HTF-Mode token NICHT als WD_LTF nehmen)
    wd_ltf = None
    tf_map = {
        "H1": "H1", "1H": "H1",
        "H4": "H4", "4H": "H4",
...
def compute_tp_sl(
    ...
    """
    Returns: (tp_price, sl_price, rr_pos, sl_dist)
    rr_pos ist immer positiv (Gewinn-RR), rr_signed wird später gesetzt.
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
    ...
    if direction == "long":
        ...
        baseline_sl = pivot_low - 0.1 * rng  # 10% unter Pivot-Range (wie vorher)
    ...
    # SL so wählen, dass RR nicht > RR_MAX wird (=> SL mindestens tp_dist/RR_MAX)
    ...
    return float(tp_price), float(sl_price), float(rr), float(sl_dist)
...
def simulate_trade(
    ...
    """
    Time-Stop:
      sobald time >= entry_time + max_hold_days -> Exit auf Candle-Open.
      rr_signed dann = (Preisbewegung / sl_dist), kann <0 oder >0 sein.
    """
    entry_time = pd.Timestamp(ltf.iloc[entry_idx]["time"])
    deadline = entry_time + pd.Timedelta(days=int(max_hold_days))
    ...
    for j in range(entry_idx + 1, len(ltf)):
        ...
        # Time-Stop zuerst prüfen (Exit auf Open der ersten Candle NACH/BEI Deadline)
        if t >= deadline:
            if direction == "long":
                rr_signed = (o - entry_price) / sl_dist
                res = "win" if rr_signed > 0 else "loss" if rr_signed < 0 else "flat"
            else:
                rr_signed = (entry_price - o) / sl_dist
                res = "win" if rr_signed > 0 else "loss" if rr_signed < 0 else "flat"
            return {
                ...
                "exit_price": float(o),
                "result": res,
                "rr_signed": float(rr_signed),
                "exit_reason": "time_stop",
            }

        # Normaler SL/TP Ablauf
        if direction == "long":
            hit_sl = l <= sl_price
            hit_tp = h >= tp_price
        else:
...
def add_concurrency_column(df_trades: pd.DataFrame) -> Tuple[pd.DataFrame, int, Optional[pd.Timestamp]]:
    """
    Fügt Spalte active_trades_at_entry hinzu:
    = Anzahl Trades, die zum Entry-Zeitpunkt noch offen sind (exit_time > entry_time),
      plus der neue Trade selbst.

    Liefert außerdem:
    - peak_concurrent (max)
    - peak_time (Zeitpunkt, an dem Peak erreicht wird; erste Auftretenszeit)
    """
    if df_trades.empty:
        out = df_trades.copy()
        out["active_trades_at_entry"] = []
        return out, 0, None
...
# ============================================================
# TRADES BUILDER – FULL SCRIPT (2/2)
# (setzt nahtlos Teil 1 fort)
# ============================================================
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            tp_price, sl_price, rr_pos, sl_dist = compute_tp_sl(
...
def summarize_trades(
    ...
    if peak_time is not None and pd.notna(peak_time):
        peak_time_str = pd.Timestamp(peak_time).isoformat()
    ...
    all_row = {
        ...
        "peak_concurrent_trades": int(peak_concurrent),
        "peak_concurrent_time": peak_time_str,
        "RR_MIN": float(RR_MIN),
        "RR_MAX": float(RR_MAX),
        "MAX_HOLD_DAYS": int(MAX_HOLD_DAYS),
    }
    ...
    print(f"Peak Concurrent Trades: {int(peak_concurrent)} @ {peak_time_str or 'n/a'}")
...
def main() -> None:
    ...
    for mode in ["3D", "W", "2W", "M"]:
        ...
        for (m, variant, wd_ltf, wd_path) in wd_files:
            ...
            # Summary speichern (inkl Peak Concurrent)
