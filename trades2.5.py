...
# -----------------------------------
RR_MIN = 0.90  # ✅ geändert von 0.95 -> 0.90
RR_MAX = 1.49
MIN_SL_PIPS = 50  # Fallback, falls keine Vola-Daten (oder wenn zu wenig Historie)

# ✅ Finaler Filter (ganz am Ende) auf "geplante Risikoeinheiten" (planned_rr)
RR_FILTER_MIN = 0.90
RR_FILTER_MAX = 1.30
...
def _parse_wd_meta_from_dirname(dirname: str) -> Tuple[Optional[str], str, Optional[str]]:
    ...
    # 1) Bevorzuge explizite LTF-Tokens (H1/H4/H12/D1/3D), aber ignoriere den HTF-Mode selbst.
    for t in tokens:
        if t in tf_map:
            cand = tf_map[t]
...
def compute_tp_sl(
    ...
    """
    ✅ Liefert weiterhin (tp_price, sl_price, planned_rr_pos)

    planned_rr_pos = positives RR basierend auf den initial geplanten Distanzen
    (tp_dist / sl_dist), unabhängig davon, ob Trade am Ende Win oder Loss wird.
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
    ...
    # exakt wie vorher: SL-Dist wird ggf. angehoben, um RR_MAX nicht zu überschreiten
...
def simulate_trade(
    ...
    tp_price: float,
    sl_price: float,
    rr_pos: float,  # hier weiterhin positives geplantes RR
) -> Optional[dict]:
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # ✅ rr_pos ist jetzt "planned_rr_pos" (positiv) und repräsentiert die geplanten Risikoeinheiten
            ...
            trades.append(
                {
                    ...
                    # ✅ NEU: geplante Risikoeinheiten (immer positiv, entspricht der "geplanten RR")
    ...
    # ✅ FINALER FILTER: NUR HIER filtern, und zwar auf planned_rr (nicht rr_signed)
    if not df_out.empty:
        df_out = df_out[
...
def summarize_trades(df_trades: pd.DataFrame, out_path: Path) -> None:
    ...
    for (pair, mode, variant, wd_ltf, entry_ltf), g in df.groupby(
        ...
        # ✅ optional: planned_rr ausgeben (damit du direkt siehst, wie stark der neue Filter wirkt)
        ...
        rows.append(
            {
                ...
                "avg_planned_rr": avg_planned_rr,  # ✅ neu
            }
        )
                ...
                [
                    {
                        ...
                        "avg_planned_rr": avg_planned_rr_tot,  # ✅ neu
                    }
                ]
            ),
        ],
