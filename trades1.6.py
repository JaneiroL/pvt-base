...
# -----------------------------------
# Dynamische MIN_SL Pips je HTF-Mode
# -----------------------------------
def _weighted_last_n_days_sum(hist: pd.DataFrame, weights: List[float]) -> Optional[float]:
    ...
    # wir wollen weights newest->older => invertiere tail
...
def get_min_sl_pips_from_vola_by_mode(
    ...
    """
    MIN-SL in Pips aus Vola daily:
      3D: 1 Tag Summe
      W : 2.5 Tage = last1 + last2 + 0.5*last3   (und optional * Weekly-Faktor)
      2W: 2 Tage Summe
      M : 3 Tage Summe (wie vorher)
    ...
    if mode == "3D":
        val = _weighted_last_n_days_sum(hist, [1.0])
    ...
    if mode == "M":
        val = _weighted_last_n_days_sum(hist, [1.0, 1.0, 1.0])
        return val if val is not None else MIN_SL_PIPS

    if mode == "W":
        # 2.5 Tage
        val = _weighted_last_n_days_sum(hist, [1.0, 1.0, 0.5])
        if val is None:
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # ✅ NEU: MIN-SL in Pips abhängig vom HTF-Mode:
            dyn_min_sl_pips = get_min_sl_pips_from_vola_by_mode(
                vola_map=vola_map,
                pair6=pair6,
                entry_time=entry_time,
                mode=mode,                 # 3D/W/2W/M
                weekly_factors=weekly_factors,  # nur relevant für W
            )
