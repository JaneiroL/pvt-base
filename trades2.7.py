...
# RR Gate (nur planned_rr)  ✅ Early Gate, aber Pivot wird konsumiert wie beim alten Post-Filter
RR_FILTER_MIN = 0.90
RR_FILTER_MAX = 1.30
...
def run_trades_for_wd_dfs(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        # ✅ neue Semantik: Pivot wird konsumiert, sobald ein Entry-Kandidat entsteht
        ...
        for _, row in g.iterrows():
            ...
            trade_accepted = False  # optional, nur für Klarheit
            ...
            # ✅ Entry-Kandidat entstanden -> Pivot wird konsumiert (wie beim alten Post-Filter Verhalten)
            pivot_consumed = True

            # ✅ EARLY planned_rr Gate: wenn fail, NICHT journaln, aber Pivot bleibt konsumiert
            if planned_rr_pos < RR_FILTER_MIN or planned_rr_pos > RR_FILTER_MAX:
                break  # keine weiteren Zonen mehr prüfen
            ...
            if sim is None:
                # optional: identisch zum bisherigen Verhalten bleibt Pivot konsumiert -> keine weiteren Zonen
                break
            ...
            if trade_accepted:
                break  # Pivot ist verbraucht, wir haben geloggt
    ...
    # ❌ Post-Filter entfernt: durch Gate + Pivot-Consumption entstehen nur noch gültige Trades im Journal.
    return df_out
