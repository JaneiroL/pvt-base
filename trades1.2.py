...
# Deprecation-Warnung für is_datetime64tz_dtype unterdrücken
...
def load_wickdiff_csv(path: Path, mode: str, variant: str, wd_ltf: str) -> pd.DataFrame:
    """
    Lädt eine Wickdiff-CSV und vereinheitlicht die wichtigsten Felder.
    """
    ...
    first_candle_col = _pick_any(
        df_raw,
        [
            "pivot_first_time",
            "first_candle_time",
            "htf_first_candle_time",
            "weekly_first_candle_time",
            "3day_first_candle_time",
            "2w_first_candle_time",
            "m_first_candle_time",
        ],
    )
    second_candle_col = _pick_any(
        df_raw,
        [
            "pivot_second_time",
            "second_candle_time",
            "htf_second_candle_time",
            "weekly_second_candle_time",
            "3day_second_candle_time",
            "2w_second_candle_time",
            "m_second_candle_time",
        ],
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "pivot_touch_time": (
                to_naive_datetime(df_raw[touch_col]) if touch_col else pd.NaT
            ),
    ...
    if htf_wd_low_col and htf_wd_high_col:
        df["htf_wick_diff_low"] = pd.to_numeric(
            df_raw[htf_wd_low_col], errors="coerce"
        )
        df["htf_wick_diff_high"] = pd.to_numeric(
            df_raw[htf_wd_high_col], errors="coerce"
        )
    else:
...
def load_vola_index(base: Path) -> Dict[str, pd.DataFrame]:
    ...
    if not xlsx_path.exists():
        print(
            f"⚠️ Keine Vola-Datei gefunden: {xlsx_path} – benutze statische {MIN_SL_PIPS} Pips."
        )
        return {}
    ...
    try:
        ...
        print(
            f"⚠️ Fehler beim Laden der Vola-Datei {xlsx_path}: {e} – benutze statische {MIN_SL_PIPS} Pips."
        )
        return {}
    ...
    if not vola_map:
        print(
            f"⚠️ Konnte keine gültigen Vola-Sheets in {xlsx_path} finden – benutze statische {MIN_SL_PIPS} Pips."
        )

    return vola_map

def get_min_sl_pips_from_vola(
    vola_map: Dict[str, pd.DataFrame], pair6: str, entry_time: pd.Timestamp
) -> float:
    """
    Mindest-SL-Größe in Pips:
    Summe der letzten 3 Vola-Werte VOR Entry-Datum.
...
def compute_pivot_tp_level(
    pivot_low: float, pivot_high: float, direction: str
) -> Optional[float]:
...
def is_outside_htf_zone(row: pd.Series, pivot_low: float, pivot_high: float) -> bool:
    """
    Nur für OUTSIDE-Varianten genutzt. Falls keine htf_wick_diff_* Spalten vorhanden sind,
    wird 'True' zurückgegeben (wir vertrauen dann auf Step2).
    """
...
def find_entry_candle(
    ...
    """
    Sucht erste gültige Entry-Kerze.
    NEU: Jede Kerze mit Close innerhalb [zone_low, zone_high] invalidiert diese WD.
    """
    ...
    for _, row in df.iterrows():
        ...
        # NEU: Close innerhalb der WD-Zone invalidiert diese WD (kein Entry)
        if lo <= c <= hi:
            return None
...
def compute_tp_sl(
    ...
    """
    TP wie gehabt am 1:1-Pivot-Level,
    SL = max(baseline-SL, dynamische Mindestpips aus Vola, TP/RR_MAX).
    min_sl_pips: bereits in Pips, z.B. Summe der letzten 3 Tageskerzen.
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # OUTSIDE-Check nur für OUTSIDE-Varianten
            if variant.upper() == "OUTSIDE":
                if not is_outside_htf_zone(row, pivot_low, pivot_high):
                    continue

            # Pivot 1:1-TP schon vor WD erreicht => WD ungültig
            ...
            if entry_idx is None:
                # WD invalide (Kill-Kerze oder Close-in-Zone); Pivot bleibt aktiv
                continue

            entry_time = ltf_df.loc[entry_idx, "time"]
            entry_price = float(ltf_df.loc[entry_idx, "close"])

            # dynamische Mindest-SL-Größe in Pips (3-Tages-Vola)
            dyn_min_sl_pips = get_min_sl_pips_from_vola(vola_map, pair6, entry_time)
...
def summarize_trades(df_trades: pd.DataFrame, out_path: Path) -> None:
    ...
    for (pair, mode, variant, wd_ltf, entry_ltf), g in df.groupby(
        ...
        avg_rr_win = (
            float(g.loc[g["is_win"], "rr_signed"].mean()) if wins else float("nan")
        )
        avg_rr_loss = (
            float(g.loc[~g["is_win"], "rr_signed"].mean()) if losses else float("nan")
        )
        rows.append(
            {
                "pair": pair,
    ...
    print(
        f"Gesamt: {n_tot} Trades | Wins: {wins_tot} | Losses: {losses_tot} | Win-Rate: {win_rate_tot:.1f}%"
    )
    print(
        f"Ø RRR Gewinn: {avg_rr_win_tot:.3f} | Ø RRR Verlust: {avg_rr_loss_tot:.3f} | Erwartungswert: {avg_rr_tot:.3f}"
    )
...
def normalize_ltf_input(raw: str) -> str:
    """
    Macht aus user input sowas wie:
      '4h', '4 H', 'H4', '240' => 'H4'
      '1h', 'h1', '60' => 'H1'
      '12h' => 'H12'
      'd1', 'day' => 'D1'
      '3d', 'd3' => '3D'
    oder gibt 'AUTO' / '' zurück für Automodus.
    """
    if raw is None:
        return ""
...
def main() -> None:
    ...
    # Vola-Index laden (für dynamische Mindest-SL-Größe)
    vola_map = load_vola_index(base)

    # LTF-Auswahl vom User
    ...
    if user_ltf_norm == "AUTO":
        ...
        if user_ltf_norm not in tf_map:
            raise ValueError(
                f"Unbekanntes LTF '{user_ltf_norm}'. Erlaubt (auch in lockerer Schreibweise): H1, H4, H12, D1, 3D, AUTO."
            )
        print(f"➡️ Interpretiere Eingabe '{user_raw}' als Entry-LTF: {user_ltf_norm}")
        # Gleiches LTF für alle HTFs
        entry_specs = {mode: tf_map[user_ltf_norm] for mode in ["3D", "W", "2W", "M"]}

    # Wickdiff-Dateien einsammeln
    wd_files = discover_wickdiff_files(base)
    if not wd_files:
        return

    # Pro HTF / Variante eigene Trades & Summaries
    for mode in ["3D", "W", "2W", "M"]:
        ...
        for (m, variant, wd_ltf, wd_path) in wd_files:
            ...
            out_name = (
                f"trades_{mode}_{variant}_wd{wd_ltf}_entry{entry_ltf_label}_{stamp}.csv"
            )
            ...
            summary_path = (
                out_base
                / f"summary_{mode}_{variant}_wd{wd_ltf}_entry{entry_ltf_label}_{stamp}.csv"
            )
