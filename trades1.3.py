...
def get_min_sl_pips_from_vola(
    ...
    Mindest-SL-Größe in Pips, HTF-abhängig (deine Staffelung):
      ...
      3D : nur letzte Daily-Kerze            => 1.0 * last1
      W  : letzte + halbe von davor (1.5d)   => 1.0 * last1 + 0.5 * prev1
      2W : letzte 2 Daily-Kerzen             => sum(last2)
      M  : letzte 3 Daily-Kerzen (≈ "mal 3") => sum(last3)
    ...
    try:
        if mode == "3D":
            ...
            last2 = hist.tail(2)
            if len(last2) == 1:
                val = float(last2["vola"].iloc[-1])  # nur last1 verfügbar
            else:
                prev = float(last2["vola"].iloc[-2])
                last = float(last2["vola"].iloc[-1])
                val = last + 0.5 * prev
...
def compute_tp_sl(
    ...
    min_sl_pips: bereits in Pips, HTF-abhängig (siehe get_min_sl_pips_from_vola).
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
