...
# 28er-Universum, um Pair-Codes sicher aus Dateinamen zu lesen
PAIRS_28 = {
...
def load_pivots(path: Path, tf_tag: str) -> pd.DataFrame:
    ...
    # 6-Letter Pair-Code extrahieren (robust über PAIRS_28)
...
def pair_code_from_str(s: str) -> str:
    """
    Explizit nach 28er-Pair suchen, sonst 6er-Block.
    """
    ...
    for p in PAIRS_28:
        if p in up:
            return p
    m = re.search(r"([A-Z]{6})", up)
    return m.group(1) if m else up[:6] or str(s)


def find_ltf_files_map(ltf_dir: Path) -> dict[str, Path]:
    """
    Sucht rekursiv alle CSV/XLSX in ltf_dir und mappt sie auf 6-Letter-Paircodes.
    """
    mp: dict[str, Path] = {}
...
def run_mode(
    ...
    if not all_rows:
        print(f"⚠️ Keine gültigen Wick-Differences gefunden für {mode} "
              "(Richtung, komplett im Gap, ≤19%, Unberührtheit-Regel).")
        if skipped:
    ...
    if mode == "3D":
        ...
        # nur Spalten umbenennen, die es wirklich gibt
