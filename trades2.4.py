...
# 👉 DAS ist dein neuer Filter (Planung, nicht Ergebnis!)
RR_PLANNED_FILTER_MIN = 0.9
RR_PLANNED_FILTER_MAX = 1.3
...
# ============================================================
# 🔴 WICHTIGSTE ÄNDERUNG: compute_tp_sl()
# ============================================================
...
def compute_tp_sl(
    ...
    RETURNS:
        tp_price
        sl_price
        planned_rr   <-- NEU (DAS ist dein Filterwert)
        sl_dist
    """
    ...
    # SL bleibt wie vorher
    sl_dist = max(sl_dist_baseline, min_sl_dist, tp_dist / RR_EXEC_MAX)

    # 👉 GEPLANTES RR (VOR Simulation)
    planned_rr = tp_dist / sl_dist

    # ❗ FILTER HIER (NICHT AUF ERGEBNIS)
    if not (RR_PLANNED_FILTER_MIN <= planned_rr <= RR_PLANNED_FILTER_MAX):
        return None, None, None, None

    sl_price = entry_price - sl_dist if direction == "long" else entry_price + sl_dist
    return tp_price, sl_price, planned_rr, sl_dist
# ============================================================
# TRADES BUILDER – FULL SCRIPT (2/3)
# (Teil 2 von 3 – direkt UNTER Teil 1 einfügen)
# ============================================================
...
# -----------------------------------
# Wickdiff metadata from dirnames
# -----------------------------------
...
# -----------------------------------
# Vola Index / Factors (unverändert)
# -----------------------------------
...
def _sum_last_days_fractional(hist: pd.DataFrame, days: float) -> Optional[float]:
    ...
    tail = hist.tail(need)["vola"].to_numpy(dtype=float)
...
# -----------------------------------
# Pivot-TP-Invalidation (unverändert)
# -----------------------------------
...
# -----------------------------------
# Candle classification helpers (für WD Exception-Logik)
# -----------------------------------
def _is_entry_candle_for_zone(o: float, h: float, l: float, c: float, lo: float, hi: float, direction: str) -> bool:
    """
    gleiche Logik wie find_entry_candle, aber für genau 1 Candle
    """
    # Body-cross invalidation
    if direction == "long":
    ...
    if direction == "long":
        return bool(touched and (c > hi) and (body_low >= hi))
    else:
        return bool(touched and (c < lo) and (body_high <= lo))

def _wd_invalidated_by_close(c: float, lo: float, hi: float, direction: str) -> bool:
    """
    Long: close unter der Zone invalidiert obere WD (und damit Pivot)
    Short: close über der Zone invalidiert untere WD (und damit Pivot)
    """
    if direction == "long":
        return c < lo
    return c > hi
...
# -----------------------------------
# Simulation (mit Time-Stop nach MAX_HOLD_DAYS)
# -----------------------------------
def simulate_trade(
    ltf: pd.DataFrame,
    entry_idx: int,
    ...
    """
    rr_pos = planned_rr (positiv)
    rr_signed:
      - win -> +rr_pos
      - loss -> -1.0
      - time_exit -> signed rr based on exit_price (relative to SL distance)
    """
    entry_time = pd.Timestamp(ltf.iloc[entry_idx]["time"])
    max_exit_time = entry_time + pd.Timedelta(days=int(max_hold_days))
    ...
    for j in range(entry_idx + 1, len(ltf)):
        ...
        # Time stop BEFORE checking next bars beyond max
        if t >= max_exit_time:
            # exit at candle open of this bar
            exit_price = float(row["open"])
            if direction == "long":
                rr_now = (exit_price - entry_price) / sl_dist
            else:
                rr_now = (entry_price - exit_price) / sl_dist
        ...
        # worst-case tie -> SL first (wie vorher)
...
# -----------------------------------
# WD priority ordering + Candle scan for special INV+VAL case
# -----------------------------------
def _wd_priority_sort_key(row: pd.Series, direction: str) -> float:
    zl = float(min(row["wd_low"], row["wd_high"]))
    zh = float(max(row["wd_low"], row["wd_high"]))
    return (-zh) if direction == "long" else zl

def _scan_candles_for_first_touch_and_special(
    ltf: pd.DataFrame,
    wd_rows_sorted: List[pd.Series],
    direction: str,
    start_time: pd.Timestamp,
    end_time: pd.Timestamp,
) -> Tuple[Optional[int], Optional[pd.Series], bool]:
    df = ltf[(ltf["time"] >= start_time) & (ltf["time"] <= end_time)].reset_index()
    if df.empty or not wd_rows_sorted:
        return None, None, False

    top_row = wd_rows_sorted[0]
    top_lo = float(min(top_row["wd_low"], top_row["wd_high"]))
    top_hi = float(max(top_row["wd_low"], top_row["wd_high"]))

    for _, r in df.iterrows():
        idx_ltf = int(r["index"])
        o = float(r["open"]); h = float(r["high"]); l = float(r["low"]); c = float(r["close"])

        touched_rows: List[pd.Series] = []
        for wdrow in wd_rows_sorted:
            lo = float(min(wdrow["wd_low"], wdrow["wd_high"]))
            hi = float(max(wdrow["wd_low"], wdrow["wd_high"]))
            if (h >= lo) and (l <= hi):
                touched_rows.append(wdrow)

        if not touched_rows:
            continue

        entry_candidates: List[pd.Series] = []
        for wdrow in touched_rows:
            lo = float(min(wdrow["wd_low"], wdrow["wd_high"]))
            hi = float(max(wdrow["wd_low"], wdrow["wd_high"]))
            if _is_entry_candle_for_zone(o, h, l, c, lo, hi, direction):
                entry_candidates.append(wdrow)

        top_invalid = _wd_invalidated_by_close(c, top_lo, top_hi, direction)
        special_event = bool(top_invalid and len(entry_candidates) > 0)

        if len(entry_candidates) > 0:
            if direction == "long":
                entry_wd = sorted(
                    entry_candidates,
                    key=lambda w: float(min(w["wd_low"], w["wd_high"]))
                )[0]
            else:
                entry_wd = sorted(
                    entry_candidates,
                    key=lambda w: float(max(w["wd_low"], w["wd_high"])),
                    reverse=True
                )[0]
            return idx_ltf, entry_wd, special_event
    ...
    return None, None, False
# ============================================================
# TRADES BUILDER – FULL SCRIPT (3/3)
# (Teil 3 von 3 – direkt UNTER Teil 2 einfügen)
# ============================================================
...
# Concurrency Helpers
# -----------------------------------
def add_concurrency_column(df_trades: pd.DataFrame) -> Tuple[pd.DataFrame, int, Optional[pd.Timestamp]]:
    """
    active_trades_at_entry:
    = Anzahl Trades, die zum Entry-Zeitpunkt noch offen sind (exit_time > entry_time),
      plus der neue Trade selbst.

    Liefert:
    - peak_concurrent
    - peak_time (erste Zeit, wo Peak erreicht wird)
    """
    if df_trades.empty:
        df_trades = df_trades.copy()
        df_trades["active_trades_at_entry"] = []
        return df_trades, 0, None
    ...
    for _, r in df.iterrows():
        ...
        current_active_including_this = len(heap) + 1
        active_counts.append(current_active_including_this)

        heapq.heappush(heap, pd.Timestamp(r["exit_time"]))

        if current_active_including_this > peak:
            peak = current_active_including_this
            peak_time = et

    df["active_trades_at_entry"] = active_counts
    return df, int(peak), peak_time

# -----------------------------------
# Summary (Peak-Concurrency integriert)
# -----------------------------------
def summarize_trades(df_trades: pd.DataFrame, out_path: Path, peak_conc: int, peak_time: Optional[pd.Timestamp]) -> None:
    if df_trades.empty:
    ...
    tail = pd.DataFrame(
        [
            ...
            {
                "pair": "PEAK",
                "mode": "PEAK_CONCURRENCY",
                "wd_variant": "",
                "wd_source_ltf": "",
                "entry_ltf": "",
                "n_trades": int(peak_conc),
                "wins": "",
                "losses": "",
                "avg_rr": "",
                "win_rate_%": "",
                "avg_rr_win": "",
                "avg_rr_loss": f"{peak_time}" if peak_time is not None else "",
            },
        ]
    )

    summary = pd.concat([summary, tail], ignore_index=True)
    ...
    print(f"Ø RR Win: {avg_rr_win_tot:.3f} | Ø RR Loss: {avg_rr_loss_tot:.3f} | Erwartungswert: {avg_rr_tot:.3f}")
    print(f"Peak Concurrent Trades: {peak_conc} @ {peak_time}")
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        # WD rows in correct priority order (top first for long, bottom first for short)
        g_rows = list(g.itertuples(index=False))
        # Convert to Series list for helpers
        g_series = [g.iloc[i] for i in range(len(g))]
        g_series_sorted = sorted(g_series, key=lambda r: _wd_priority_sort_key(r, direction))

        for row in g_series_sorted:
            if pivot_traded:
                break
            ...
            # Candle scan (entry + special INV+VAL)
            entry_idx, entry_wd_row, special_event = _scan_candles_for_first_touch_and_special(
                ltf=ltf_df,
                wd_rows_sorted=g_series_sorted,
                direction=direction,
                start_time=pivot_touch_time,
                end_time=end_time,
            )
            if entry_idx is None or entry_wd_row is None:
                continue

            entry_time = pd.Timestamp(ltf_df.loc[entry_idx, "time"])
            ...
            # 🔥 HERE: compute_tp_sl now filters on PLANNED RR (0.9..1.3)
            tp_price, sl_price, planned_rr, sl_dist = compute_tp_sl(
                entry_price=entry_price,
                pivot_low=pivot_low,
                pivot_high=pivot_high,
                direction=direction,
            ...
            trades.append(
                {
                    ...
                    "wd_first_time": entry_wd_row["wd_first_time"],
                    "wd_second_time": entry_wd_row["wd_second_time"],
                    "wd_low": float(entry_wd_row["wd_low"]),
                    "wd_high": float(entry_wd_row["wd_high"]),
                    ...
                    # ✅ NEW: planned_rr saved for transparency
                    "planned_rr": float(planned_rr),
                    "wickdiff_special": ("INV-VAL" if special_event else ""),
...
def main() -> None:
    ...
    for mode in ["3D", "W", "2W", "M"]:
        ...
        for (m, variant, wd_ltf, wd_path) in wd_files:
            ...
            trades_with_conc, peak_conc, peak_time = add_concurrency_column(trades)

            out_name = f"trades_{mode}_{variant}_wd{wd_ltf}_entry{entry_ltf_label}_{stamp}.csv"
            out_path = out_base / out_name
            trades_with_conc.to_csv(out_path, index=False)
            ...
            summarize_trades(trades_with_conc, summary_path, peak_conc, peak_time)
