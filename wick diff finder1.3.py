...
# NEW: Higher-Timeframe-Ordner (für harte Zeitgrenzen)
HTF_DATA_DIRS = {
    "3D": Path("time frame data") / "3D",
...
# -----------------------------
# NEW: HTF window end derivation (harte Grenze)
# -----------------------------
...
def derive_window_end_from_htf(
    ...
    # no next candle -> end at data end
...
def run_mode_variant(base: Path, mode: str, ltf_key: str, variant: str) -> pd.DataFrame:
    ...
    # NEW: HTF file map (für harte Endgrenze)
    ...
    for _, r in piv.iterrows():
        ...
        # =========================
        # NEW WINDOW:
        # win_start = start of candle #1
        # win_end   = end of candle #2 (hard boundary)
        # =========================
        win_start = htf_t1
        ...
        if end2 is None:
            # clean fallback (NO dur-logic)
            if pair6 not in warned_end:
                print(f"⚠️ Konnte HTF-Ende (2. Candle) nicht aus HTF-Daten ableiten für {mode}/{pair6}. "
