...
# -----------------------------------
# Dynamische MIN_SL Pips je HTF-Mode (0.1-Schritte, inkl. 2.5 Tage etc.)
# -----------------------------------
def _sum_last_days_fractional(hist: pd.DataFrame, days: float) -> Optional[float]:
    ...
    Unterstützt 0.1-Schritte:
      ...
      2.5 -> last1 + last2 + 0.5*last3
      0.1 -> 0.1*last1
    hist: muss bereits date < entry_date gefiltert und nach date sortiert sein.
    """
    if days is None:
        return None
    days = float(days)
    ...
    if frac > 1e-12:
        # letzter weight gehört zur "zusätzlichen" ältesten Kerze in diesem Tail
        weights[-1] = frac
...
def get_min_sl_pips_from_vola_by_mode(
    ...
    """
    Original-Logik: MIN-SL in Pips wird aus Daily-Vola-Summe der letzten X Tage berechnet.
    X kommt pro HTF-Mode aus vola_days_by_mode (0.1-Schritte möglich, z.B. 2.5).
    Kein Weekly-Toggle, kein extra Faktor-Gedöns.
    """
    ...
    hist = df[df["date"] < entry_date].sort_values("date")
    if hist.empty:
        return MIN_SL_PIPS

    mode = str(mode).upper().strip()
    days = vola_days_by_mode.get(mode, 0.0) if vola_days_by_mode else 0.0
...
def run_trades_for_wd_file(
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # MIN-SL in Pips abhängig vom HTF-Mode (User-Input in 0.1 Schritten)
...
def main() -> None:
    ...
    # VOL-Konfiguration (0.1-Schritte). Genau hier gibst du z.B. 2,5 für Weekly ein.
    defaults = {"3D": 45.0, "W": 2.0, "2W": 2.0, "M": 3.0}
    print("\nVOL-Konfiguration (Daily-Vola-Tage für MIN-SL).")
    print("  Eingabe in 0,1 Schritten (z.B. 2,5).")
    ...
    print("\n✅ VOL-Settings aktiv:")
    for m in ["3D", "W", "2W", "M"]:
        print(f"  {m}: {vola_days_by_mode[m]:.1f} Tage")
