...
# Sonder-Fixes für kaputte/verkürzte Pair-Codes in Dateinamen (aus deinem System)
# (Beispiele: "OANDAG..." tauchte als kaputter Prefix auf und war eigentlich GBPNZD)
SPECIAL_PAIR_FIX = {
    "OANDAG": "GBPNZD",
}
...
def infer_pair_from_text(txt: str):
    """
    Erst: SPECIAL_PAIR_FIX, dann: 28er-Set, dann: generischer 6-Letter.
    """
    up_raw = str(txt).upper()
    up = re.sub(r"[^A-Z]", "", up_raw)

    # 0) Sonderfixe (kaputte Codes)
...
def detect_pivots_for_pair(pair: str, timeframe: str, df: pd.DataFrame, start_dt, end_dt) -> pd.DataFrame:
    # Weekly/3D/2W/M: Logik identisch, nur andere Candles
    if start_dt:
    ...
    for i in range(len(df) - 1):
        ...
        # Pivot-Typ via Candle-Farbwechsel
        ...
        # Pivot-Linie = open der 2. Candle
        chosen_price = float(o2)

        # Große Pivot-Gap (wie vorher)
        if ptype == "long":
        ...
        # Große Wick-Difference (deine korrigierte Definition)
        # LONG: WD liegt unterhalb/innerhalb des Gap, obere WD-Grenze = höherer der beiden unteren Wicks
        # SHORT: WD liegt oberhalb/innerhalb des Gap, untere WD-Grenze = tieferer der beiden oberen Wicks
        if ptype == "long":
        ...
        # Clamp auf Gap, damit WD garantiert im Gap liegt
        wick_diff_low = max(wick_diff_low, gap_low_n)
        wick_diff_high = min(wick_diff_high, gap_high_n)
        wd_lo = min(wick_diff_low, wick_diff_high)
        wd_hi = max(wick_diff_low, wick_diff_high)
        wick_diff_low, wick_diff_high = wd_lo, wd_hi

        # First touch der großen Pivot-Gap (Info für Block 2/3)
...
def parse_tf_input(s: str) -> set[str]:
    ...
    for t in tokens:
        if t in {"w", "week", "weekly", "1w"}:
            ...
            out.add("3D")
        elif t in {"2w", "2week", "2weekly", "2-weekly", "2wweekly"} or "2weekly" in t:
            out.add("2W")
...
def collect_files_for_tf(base: Path, wanted: set[str]):
    ...
    for name in ["time frame data", "time_frame_data", "timeframe_data"]:
        ...
        if cand.exists() and cand.is_dir():
            ...
            break
    if tf_root is None:
        tf_root = base  # fallback

    # Aliases für Ordnernamen
    tf_dirs = {
...
def tf_to_outsub(tf: str) -> str:
    # Output-Ordnernamen exakt wie in deinem Repo (Screenshot)
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-Pair Pivot-Gap Finder (W / 3D / 2W / M / All) inkl. großer Wick-Diff."
    )
    parser.add_argument(
        "--inpath",
        type=str,
        help="Basis-Ordner (Standard: aktuelles Verzeichnis). Unterordner: 'time frame data/<TF>'.",
    )
            ...
            if path.suffix.lower() == ".csv":
                ...
                for sh_name, df in sheets.items():
                    ...
                    try:
                        ...
                        see = str(e)
                        skipped.append((f"{path.name} / {sh_name}", f"Fehler: {see}"))
                if not any_ok:
    ...
    for tf in sorted(wanted):
        ...
        path_tf = dir_tf / f"pivots_gap_ALL_{tag}_{stamp}.csv"

        # nur Zeilen dieses TF schreiben
        out[out["timeframe"] == tf].to_csv(path_tf, index=False)
        outputs.append(path_tf)
