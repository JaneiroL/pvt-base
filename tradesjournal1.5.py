...
# COT-Signale
# -----------------------------------
def load_cot_signals(cot_path: Path) -> Optional[pd.DataFrame]:
    print(f"🔄 Lade COT-Signale aus: {cot_path}")
    xls = pd.read_excel(cot_path, sheet_name=None)
    rows = []
    for sheet_name, df in xls.items():
        ...
        col_pair = find_col(["pair", "symbol"])
        col_bias = find_col(["bias", "signal", "richtung"])
        col_start = find_col(["start", "von", "begin"])
        col_end = find_col(["end", "bis", "to"])

        if not all([col_pair, col_bias, col_start, col_end]):
            continue

        for _, row in df2.iterrows():
            pair_raw = row[col_pair]
            bias_raw = str(row[col_bias]).strip().lower()
            if pd.isna(pair_raw) or bias_raw not in {"long", "short"}:
                continue
            start = pd.to_datetime(row[col_start], errors="coerce")
            end = pd.to_datetime(row[col_end], errors="coerce")
            if pd.isna(start) or pd.isna(end):
                continue
            pair6 = pair_code_from_str(str(pair_raw)).upper()
            rows.append(
                {
                    "pair6": pair6,
                    "bias": bias_raw,
    ...
    if not rows:
        print("⚠️ Konnte in der COT-Datei keine brauchbaren Signale finden.")
        return None
...
def cot_allows_trade(
    cot_df: Optional[pd.DataFrame], pair6: str, direction: str, entry_time: pd.Timestamp
) -> bool:
    if cot_df is None:
        return True
    date = pd.to_datetime(entry_time).normalize()
    mask = (
        (cot_df["pair6"] == pair6)
        & (cot_df["bias"] == direction.lower())
        & (cot_df["start"] <= date)
        & (cot_df["end"] >= date)
    )
...
def find_entry_candle(
    ...
    df = ltf[ltf["time"] >= start_time].reset_index()
    if df.empty:
        return None
...
# -----------------------------------
# Neue Pivot-Invalidation: Bounce bis TP ohne Wicktouch
# -----------------------------------
def pivot_invalid_by_bounce(
    ltf: pd.DataFrame,
    direction: str,
    pivot_low: float,
    pivot_high: float,
    ...
    """
    Regel:
    Ab start_time (Pivot-Touch):
      - Wenn der Markt zuerst das grobe TP-Level erreicht,
        BEVOR irgendeine Wick-Zone [zone_min, zone_max] berührt wurde,
        ist der Pivot komplett INVALID.
    """

    if pd.isna(start_time):
        return False

    lo = float(min(pivot_low, pivot_high))
    hi = float(max(pivot_low, pivot_high))
    rng = hi - lo
    if rng <= 0:
        return False

    z_lo = float(min(zone_min, zone_max))
    z_hi = float(max(zone_min, zone_max))
    ...
    df = ltf[ltf["time"] >= start_time].reset_index(drop=True)
    if df.empty:
        return False
    ...
    for idx, row in df.iterrows():
        ...
        # Wick-Zone Berührung?
        if first_zone_idx is None:
            if (h >= z_lo) and (l <= z_hi):
                first_zone_idx = idx

        # TP-Level erreicht?
        if first_tp_idx is None:
            if direction == "long":
                if h >= tp_level:
                    first_tp_idx = idx
            else:
        ...
        if first_zone_idx is not None and first_tp_idx is not None:
            break

    if first_tp_idx is not None and (first_zone_idx is None or first_tp_idx < first_zone_idx):
        # TP-Level kam zuerst → Pivot invalid
        return True
...
def run_mode(
    ...
    # Gruppiere alle Wick-Zonen nach Pivot
    grouped = wd.groupby(
        ["pair6", "pivot_type", "pivot_first_time", "pivot_second_time"],
        sort=False,
    )

    for (pair6, direction, p_first, p_second), g in grouped:
        ...
        touch_time = g["pivot_touch_time"].iloc[0] if "pivot_touch_time" in g.columns else pd.NaT

        zone_min = float(g["wd_low"].min())
        zone_max = float(g["wd_high"].max())

        start_for_bounce = touch_time if pd.notna(touch_time) else p_second

        # Neue Bounce-Invalidation
        if pivot_invalid_by_bounce(
            ltf_df,
            direction,
            pivot_low,
            pivot_high,
            ...
            # kompletter Pivot invalid → keine Trades aus dieser Gruppe
            continue

        # Nun versuchen wir, EINEN Trade aus den Wick-Zonen dieses Pivots zu bauen
        g_sorted = g.sort_values("wd_first_time")
        trade_done = False
        ...
        for _, row in g_sorted.iterrows():
            ...
            start_time = row["pivot_touch_time"]
            entry_idx = find_entry_candle(
                ltf_df, row["wd_low"], row["wd_high"], direction, start_time
            )
            ...
            if not cot_allows_trade(cot_df, pair6, direction, entry_time):
                continue
            ...
            trade_done = True  # max. 1 Trade pro Pivot
...
def main() -> None:
    ...
    # COT-Datei suchen (case-insensitive nach 'cot' & 'signal')
    ...
    if cot_dir.exists():
        candidates = sorted(
            p
            for p in cot_dir.glob("*.xlsx")
            if "cot" in p.name.lower() and "signal" in p.name.lower()
        )
        if candidates:
    ...
    if cot_path is None:
        ...
        cot_df = load_cot_signals(cot_path)
