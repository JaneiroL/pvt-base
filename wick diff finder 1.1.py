...
MODE_SPECS = {
    ...
    "M":  {"pivot_subdir": "Monthly", "default_ltf": "3D"},  # wie vorher: M-Pivots -> 3D-LTF
}
...
LTF_DIRS = {
    ...
    "3D":  Path("time frame data") / "3D",   # nur Default für M, wird nicht im Prompt angezeigt
}
...
def to_dt(s):
    """
    Robust gegen gemischte Zeitzonen:
    - immer erstmal mit utc=True parsen
    - dann Zeitzone entfernen => alles tz-naiv
    """
...
# --- OHLC-Reader (robust, wie im PivotFinder) ---
...
def detect_ltf_wickdiffs(
    ...
    if ltf.empty:
        return []
    df = ltf.copy()
    # sicherstellen, dass df["time"] tz-naiv ist
    df["time"] = to_dt(df["time"])
...
def parse_ltf_input(s: str) -> Optional[List[str]]:
    """
    LTF-Parsing:
      - leer => None (verwende Default pro HTF)
      - 'All' => ['H1','H4','H12','D1']
      - sonst Kombis: H1,H4,H12,D1 (mit Aliases)
    """
    if s is None:
        return None
    raw = s.strip().lower()
...
def main():
    ...
    # --- HTF-Prompt, falls nichts angegeben ---
    if not args.tf:
    ...
    # --- LTF-Prompt (einmal, gilt für alle HTFs) ---
    if not args.ltf:
    ...
    if ltf_list is None:
        ...
        for m in modes:
            mode_to_ltfs[m] = ltf_list[:]  # gleiche LTFs für alle HTFs
