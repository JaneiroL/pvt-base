...
# -----------------------------------
# Wickdiff Loader (VARIANTE 2 = OUTSIDE)
# -----------------------------------
...
def load_wickdiffs_outside(mode: str, base: Path) -> pd.DataFrame:
    """
    Variante 2: OUTSIDE Wickdiffs laden.
    Ordner (wie in Step2 OUTSIDE):
      W  -> outputs/wickdiffs/W→H4_OUTSIDE
      3D -> outputs/wickdiffs/3D→H1_OUTSIDE
      2W -> outputs/wickdiffs/2W→1D_OUTSIDE
      M  -> outputs/wickdiffs/M→3D_OUTSIDE
    """
    folder_map = {
        "W":  "W→H4_OUTSIDE",
        "3D": "3D→H1_OUTSIDE",
    ...
    if mode not in folder_map:
        raise ValueError(f"Unknown mode '{mode}'. Use one of: {list(folder_map)}")

    wd_dir = base / "outputs" / "wickdiffs" / folder_map[mode]
    path = _latest_wickdiff_csv_in_dir(wd_dir)

    if path is None:
        print(f"❌ Keine OUTSIDE Wick-Diffs in {wd_dir} gefunden.")
        return pd.DataFrame()

    print(f"🔄 Verwende OUTSIDE Wick-Diffs ({mode}): {path}")
    ...
    # In OUTSIDE-Step2 Output heißen sie: htf_gap_low/high und htf_wick_diff_low/high
    gap_low_col  = pick_any(["htf_gap_low", "gap_low", "weekly_gap_low", "3day_gap_low", "2w_gap_low", "m_gap_low"])
    gap_high_col = pick_any(["htf_gap_high","gap_high","weekly_gap_high","3day_gap_high","2w_gap_high","m_gap_high"])

    first_candle_col  = pick_any(["htf_first_candle_time", "first_candle_time", "weekly_first_candle_time","3day_first_candle_time","2w_first_candle_time","m_first_candle_time"])
    second_candle_col = pick_any(["htf_second_candle_time","second_candle_time","weekly_second_candle_time","3day_second_candle_time","2w_second_candle_time","m_second_candle_time"])
    ...
    # OUTSIDE-Step2 Output enthält htf_wick_diff_low/high → laden wir (für Validierung/Debug)
    htf_wd_low_col = pick_any(["htf_wick_diff_low","wick_diff_low"])
    htf_wd_high_col = pick_any(["htf_wick_diff_high","wick_diff_high"])

    first_touch_col = None
    for cand in ["htf_first_touch_time","first_touch_time","weekly_first_touch_time","3day_first_touch_time","2w_first_touch_time","m_first_touch_time"]:
        try:
    ...
    df = pd.DataFrame({
        ...
        "htf_wick_diff_low": pd.to_numeric(df_raw[htf_wd_low_col], errors="coerce"),
        "htf_wick_diff_high": pd.to_numeric(df_raw[htf_wd_high_col], errors="coerce"),
    })
    ...
    df = df.dropna(subset=[
        ...
        "htf_wick_diff_low","htf_wick_diff_high",
...
# -----------------------------------
# OUTSIDE-Validierung (Fail-fast)
# -----------------------------------
def assert_outside_zone(
    pivot_low: float, pivot_high: float,
    htf_wd_low: float, htf_wd_high: float,
    z_low: float, z_high: float,
) -> bool:
    """
    True wenn Zone innerhalb Pivot UND komplett außerhalb HTF-Wickdiff.
    """
    ...
    zl, zh = float(min(z_low, z_high)), float(max(z_low, z_high))
...
def run_mode(
    ...
    wd = load_wickdiffs_outside(mode, base)
    if wd.empty:
        return pd.DataFrame()
    ...
    if wd.empty:
        print(f"⚠️ Keine OUTSIDE Wick-Pivots nach Filter für Modus {mode}.")
        return pd.DataFrame()
    ...
    for (pair6, direction, p1, p2), g in pivot_groups:
        ...
        for _, row in g.iterrows():
            ...
            # OUTSIDE-Check (damit wir nie aus Versehen INSIDE handeln)
            if not assert_outside_zone(
                pivot_low=pivot_low, pivot_high=pivot_high,
                htf_wd_low=float(row["htf_wick_diff_low"]),
                htf_wd_high=float(row["htf_wick_diff_high"]),
                z_low=zone_low, z_high=zone_high,
            ):
                continue
...
# Summary
# -----------------------------------
def summarize_trades(df_trades: pd.DataFrame, mode: str, out_dir: Path, stamp: str) -> None:
    if df_trades.empty:
    ...
    summary_path = out_dir / f"trades_{mode}_summary_OUTSIDE_{stamp}.csv"
    summary.to_csv(summary_path, index=False)

    print(f"\n===== Zusammenfassung {mode}-Trades (OUTSIDE) =====")
...
def main() -> None:
    ...
    for mode, (ltf_dir, ltf_label) in mode_specs.items():
        trades = run_mode(base=base, mode=mode, ltf_dir=ltf_dir, ltf_label=ltf_label)
        if trades.empty:
            continue
        path = out_base / f"trades_{mode}_OUTSIDE_{stamp}.csv"
        trades.to_csv(path, index=False)
        print(f"\n💾 Trades {mode} (OUTSIDE) gespeichert in: {path.resolve()}")
