...
def load_wickdiffs(mode: str, base: Path) -> pd.DataFrame:
    """
    Lädt immer aus den richtigen Unterordnern:
      W  -> outputs/wickdiffs/W→H4
      3D -> outputs/wickdiffs/3D→H1
      2W -> outputs/wickdiffs/2W→1D
      M  -> outputs/wickdiffs/M→3D
    und nimmt die NEUESTE passende CSV (egal wie genau sie heißt).
    """
    folder_map = {
        "W":  "W→H4",
        "3D": "3D→H1",
    ...
    # Spalten robust picken (dein Step2 kann rename machen)
    pair_col = _pick_col_generic(df_raw, "pair")
    ptype_col = _pick_col_generic(df_raw, "pivot_type")

    # Pivot-Gap Spalten können je nach Mode umbenannt sein, aber in deinen Outputs sind sie i.d.R. gap_low/gap_high
    # Wir versuchen mehrere Kandidaten:
    ...
    gap_low_col  = pick_any(["gap_low", "weekly_gap_low", "3day_gap_low", "2w_gap_low", "m_gap_low"])
    gap_high_col = pick_any(["gap_high","weekly_gap_high","3day_gap_high","2w_gap_high","m_gap_high"])

    first_candle_col  = pick_any(["first_candle_time","weekly_first_candle_time","3day_first_candle_time","2w_first_candle_time","m_first_candle_time"])
    second_candle_col = pick_any(["second_candle_time","weekly_second_candle_time","3day_second_candle_time","2w_second_candle_time","m_second_candle_time"])
    ...
    for cand in ["first_touch_time","weekly_first_touch_time","3day_first_touch_time","2w_first_touch_time","m_first_touch_time"]:
        try:
...
def run_mode(
    ...
    # Modus-Laufzeitfenster (kannst du später feinjustieren)
