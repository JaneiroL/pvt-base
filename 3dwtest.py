...
def parse_tf_input(s: str):
    ...
    if s in {"3d", "3", "3tage", "3-tage"}: return {"3D"}
    if s in {"both", "all", "w+3d", "3d+w"}: return {"W","3D"}
    raise ValueError("Ungültige Eingabe. Erlaubt: W, 3D oder Both")

def prompt_if_missing(args):
    if not args.inpath:
        print("Pfad-Basis (leer = aktuelles Verzeichnis, ich suche darunter 'time frame data/W' und 'time frame data/3D'):")
...
def collect_files_for_tf(base: Path, wanted: set[str]):
    ...
    # --- Weekly Dateien ---
    if "W" in wanted:
        dir_w = existing_subdir(dir_timeframe_root / "W", dir_timeframe_root / "1W") \
    ...
    # --- 3D Dateien ---
    if "3D" in wanted:
        dir_3d = existing_subdir(dir_timeframe_root / "3D") or (dir_timeframe_root / "3D")
        if dir_3d.exists():
...
# -------- Hauptlogik --------
def main():
    parser = argparse.ArgumentParser(description="Multi-Pair Pivot-Gap Finder (Weekly / 3D / Both).")
    parser.add_argument("--inpath", type=str, help="Basis-Ordner (Standard: aktuelles Verzeichnis). Unterordner: 'time frame data/W' & 'time frame data/3D'.")
    ...
    if base.is_file():
        print("❌ Bitte einen Basis-Ordner angeben, keine einzelne Datei (ich suche selber darunter nach W/3D).")
        sys.exit(1)
