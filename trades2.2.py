...
# -----------------------------------
# ✅ Neue RR-Band-Regel: NUR Trades in diesem Band zulassen
RR_MIN = 0.50
RR_MAX = 0.95
...
def _parse_yes_no_default_yes(raw: str) -> bool:
    ...
    'y/yes/ja/1/on' => Ja
    Unbekannt => Ja (wie bisheriges Verhalten)
    """
...
def _parse_wd_meta_from_dirname(dirname: str) -> Tuple[Optional[str], str, Optional[str]]:
    ...
    # ✅ FIX: WD_LTF korrekt aus Ordnernamen lesen (z.B. "3D_H4_ALL" -> H4, NICHT 3D)
    wd_ltf = None
    tf_map = {
        "H1": "H1", "1H": "H1",
        "H4": "H4", "4H": "H4",
...
def compute_tp_sl(
    ...
    """
    ✅ Band-RR-Logik (0.50..0.95) sauber hierarchisch:

    1) TP bleibt Pivot-Measured-Move (identisch wie vorher).
    2) Baseline-SL + Min-SL bleiben als Untergrenzen.
    3) Falls RR zu hoch wäre (> RR_MAX), wird SL vergrößert (SL-Floor = tp_dist/RR_MAX).
    4) Falls RR zu niedrig wäre (< RR_MIN), ist Trade unmöglich (weil SL nicht verkleinert werden darf) -> reject.
    """
    pivot_low = float(pivot_low)
    pivot_high = float(pivot_high)
    if pivot_high <= pivot_low:
    ...
    # 1) Erstmal Baseline/MinSL-Untergrenzen
    sl_dist = max(sl_dist_baseline, min_sl_dist)

    # 2) RR darf NICHT größer als RR_MAX sein -> SL ggf. vergrößern
    #    rr = tp_dist / sl_dist  => rr <= RR_MAX  <=> sl_dist >= tp_dist / RR_MAX
    sl_floor_for_rr_max = tp_dist / float(RR_MAX)
    sl_dist = max(sl_dist, sl_floor_for_rr_max)

    # 3) RR darf NICHT kleiner als RR_MIN sein -> dann wäre SL zu groß
    #    rr >= RR_MIN <=> sl_dist <= tp_dist / RR_MIN
    sl_ceiling_for_rr_min = tp_dist / float(RR_MIN)
    if sl_dist > sl_ceiling_for_rr_min:
        return None, None, None

    rr = tp_dist / sl_dist

    # Safety-Gate (sollte durch obige Konstruktion bereits passen)
    if rr < RR_MIN or rr > RR_MAX:
        return None, None, None

    sl_price = (entry_price - sl_dist) if direction == "long" else (entry_price + sl_dist)
    return tp_price, sl_price, float(rr)
